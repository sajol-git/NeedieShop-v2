            ...
            return (
              ...
              >
                <div className="w-8 h-8 flex items-center justify-center">
                  <Icon className={`w-6 h-6 ${['Checkout', 'Track'].includes(item.name) ? 'scale-110' : item.name === 'Account' ? 'scale-100' : ''}`} />
                </div>
            ...
            <Link key={item.name} href={item.href} className="relative p-1 group">
              <div className="w-10 h-10 flex items-center justify-center">
                {item.name === 'Bag' ? (
                  ...
                  <Icon className={`w-6 h-6 text-gray-400 group-hover:text-gray-600 transition-colors ${['Checkout', 'Track'].includes(item.name) ? 'scale-110' : item.name === 'Account' ? 'scale-100' : ''}`} />
                )}
              </div>
              {item.count !== undefined && item.count > 0 && (
                <span className="absolute top-1 right-1 w-4 h-4 bg-[#8B183A] text-white text-[8px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-lg">
