...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <motion.div 
      ...
      viewport={{ once: true }}
      className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-500 p-1.5"
    >
      ...
      <div className="px-2 py-3">
        <Link href={`/products/${product.slug}`}>
          <h3 className="text-sm font-bold text-gray-900 mb-1.5 line-clamp-2 leading-tight group-hover:text-[#8B183A] transition-colors">
            {product.name}
          </h3>
        </Link>
